...
async function saveSheetChanges(updates) {
  ...
  try {
    ...
    const valueRanges = updates.map(up => {
      ...
      const rows = isInv ? dataResult.inventory : dataResult.sales;
...
async function appendSheetRow(tab, rowData) {
  ...
  if (client.isMock) {
    const data = readMockDb();
    const targetTab = tab.toLowerCase().includes('inventory') ? 'inventory' : 'sales';
    const targetTable = data[targetTab];
    const headers = targetTable[0];
    
    const isInv = tab.toLowerCase().includes('inventory');
    const nameColIndex = findHeaderIndex(headers, "Item name");
    
    let existingRowIndex = -1;
    let existingItemRow = null;
    const inputName = String(rowData["Item name"] || rowData["name"] || "").trim().toLowerCase();
  ...
  try {
    ...
    const isInv = tab.toLowerCase().includes('inventory');
    const actualTab = isInv ? invTab : salTab;
    const headers = isInv ? (dataResult.inventory[0] || []) : (dataResult.sales[0] || []);
    ...
    if (isInv && inputName) {
      for (let i = 1; i < dataResult.inventory.length; i++) {
        const row = dataResult.inventory[i];
        if (row && row[nameColIndex] && String(row[nameColIndex]).trim().toLowerCase() === inputName) {
